...
import 'package:vnl_common_ui/shadcn_flutter.dart' hide VNLSlider;
...
void main() {
  testWidgets('every catalog item renders from its exampleData without throwing', (
    ...
    expect(find.text('Press me'), findsOneWidget); // VNLButton -> Text child
  });

  testWidgets('a live DataModel update propagates to the rendered VNLCheckbox', (
    tester,
  ) async {
    final catalog = GenCatalog.asCatalog();
    ...
    await tester.pumpAndSettle();
    expect(
      tester.widget<VNLCheckbox>(find.byType(VNLCheckbox)).state,
