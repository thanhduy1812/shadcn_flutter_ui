...
double _fallbackBackdropTransform(SheetStage stage, VNLSheetStageResolution res) {
...
abstract class SheetStage {
  ...
  SheetStage operator +(SheetStage other) => VNLAdditiveSheetStage(this, other);

  /// Difference of two stages (offsets and backdrop transforms are subtracted).
  SheetStage operator -(SheetStage other) => VNLSubtractedSheetStage(this, other);

  /// Scales this stage's offset and backdrop transform by [factor].
  SheetStage operator *(double factor) => VNLMultipliedSheetStage(this, factor);

  /// Divides this stage's offset and backdrop transform by [factor].
  SheetStage operator /(double factor) => VNLDividedSheetStage(this, factor);
...
class VNLClosedSheetStage extends SheetStage {
...
class VNLExpandedSheetStage extends SheetStage {
  /// Explicit backdrop transform for this stage; null falls back to expansion.
  final double? backdropTransform;
...
class VNLFixedSheetStage extends SheetStage {
...
class VNLFractionSheetStage extends SheetStage {
...
class VNLPeekDragHandleSheetStage extends SheetStage {
  /// Explicit backdrop transform for this stage; null falls back to expansion.
  final double? backdropTransform;
...
/// Sum of two stages.
class VNLAdditiveSheetStage extends SheetStage {
  /// The left operand.
  final SheetStage a;
...
class VNLSubtractedSheetStage extends SheetStage {
  /// The left operand.
  final SheetStage a;
...
class VNLMultipliedSheetStage extends SheetStage {
  /// The operand.
...
class VNLDividedSheetStage extends SheetStage {
  /// The operand.
  final SheetStage stage;
...
/// [VNLSheetController.stage]. Comparing it to another [SheetStage] resolves both
